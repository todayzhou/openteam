...
如果你已经是 ChatGPT、Claude、Gemini、Kimi、DeepSeek、千问等 AI 网站的会员，或者你本来就在浏览器里使用这些 AI 网站，OpenTeam 可以直接复用这些网页会话来组织多角色讨论。它不要求你额外接 OpenAI API、Claude API 或其他模型 API，也不需要为每次讨论单独消耗 API token。
...
同一个群聊内的人员可以选择不同 AI 站点，例如 Gemini、ChatGPT、Claude、DeepSeek、Kimi、千问。用户可以把同一任务发给不同模型背后的人员，比较它们的回答质量和侧重点。
...
- `src/content/sites/qwen.ts`

Kimi 和千问使用额外的 page world bridge 脚本处理页面环境写入：

- `src/content/kimiPageWorldBridge.ts`
- `src/content/qwenPageWorldBridge.ts`
